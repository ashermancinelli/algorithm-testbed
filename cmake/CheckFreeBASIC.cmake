...
macro(add_freebasic_executable)
  ...
  cmake_parse_arguments(
    ADD_FREEBASIC_EXECUTABLE "${OPTIONS}" "${SVARGS}" "${MVARGS}" ${ARGN}
  )

  list(LENGTH ADD_FREEBASIC_EXECUTABLE_SOURCES len)
  if(${len} GREATER 1)
    message(
      FATAL_ERROR "I don't have a great way to build multi-source freebasic "
                  "programs at the moment."
    )
  endif()

  list(TRANSFORM ADD_FREEBASIC_EXECUTABLE_SOURCES
       PREPEND "${CMAKE_CURRENT_SOURCE_DIR}/"
  )

  add_custom_target(
    ${ADD_FREEBASIC_EXECUTABLE_NAME} ALL
  ...
  add_custom_command(
    ...
    COMMAND ${FBC_EXE} ${ADD_FREEBASIC_EXECUTABLE_SOURCES} -x
            ${CMAKE_CURRENT_BINARY_DIR}/${ADD_FREEBASIC_EXECUTABLE_NAME}
  )
endmacro()
